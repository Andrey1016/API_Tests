...
def api_request_with_basic_auth(username, password):
    ...
    if response.status_code != 200:
        ...
        print(f"The 'count' value is: {count_value}")

        print(f"API Request Successful. Response: {count_value}")
...
# print(f"Count: {count}")
def assert_json_response(response_text):
    # Parse the JSON response
    ...
    # Assert the values in the JSON response
...
assert_json_response(response_text)
